...
contains
  ...
  subroutine glide_allocarr(model)    
    ...
    ! MJH 10/21/13:  I have modified the *_io.F90 and its autogenerated code to skip inputting or outputting
    !  netCDF variables for which the data is undefined because it is not allocated based on the current config settings.
    !  However, because some data is defined using math between multiple model variables, a check for allocation
    !  will fail on those variables.  Instead, of non allocating unneeded variables, they should be allocated 
    !  with size 0.  This is done in Fortran by specifying an end index that is smaller than the start index, 
    !  e.g. allocate(variable(1:0))
    !  Also note that in *_vars.def some data uses hardwired vertical indices.  This it not a problem for 
    !  the size intrinsic function used in *_io.F90 to check if a variable is enabled, but it will generate a runtime error
    !  if the -f-bounds-check compiler flag is enabled.  A way around this would be to allocate any disabled
    !  3-d variables with only a vertical dimension, e.g. allocate(variable(upn, 1:0, 1:0))
    ...
    if (model%options%whichdycore /= DYCORE_GLIDE) then   ! glam/glissade only
       ...
       allocate( model%temper%bfricflx(1:0, 1:0) )
       allocate( model%temper%lcondflx(1:0, 1:0) )
       allocate( model%temper%dissipcol(1:0, 1:0) )
    endif
    ...
    if (model%options%whichtemp == TEMP_ENTHALPY) then   ! enthalpy scheme (under construction)
       ...
       allocate( model%temper%waterfrac(1:0, 1:0, 1:0) )
    endif
...
!!    call coordsystem_allocate(model%general%velo_grid, model%velocity%velmask)    ! no longer used
    ...
    if (model%options%whichdycore == DYCORE_GLIDE) then
       ...
       allocate( model%velocity%wgrd(1:0, 1:0, 1:0) )
       allocate( model%velocity%diffu(1:0, 1:0) )
       allocate( model%velocity%diffu_x(1:0, 1:0) )
       allocate( model%velocity%diffu_y(1:0, 1:0) )
       allocate( model%velocity%total_diffu(1:0, 1:0) )
       allocate( model%velocity%tau_x(1:0, 1:0) )
       allocate( model%velocity%tau_y(1:0, 1:0) )
    endif
...
!       call coordsystem_allocate(model%general%velo_grid, upn, model%velocity%magres)
       ...
       ! higher-order stress arrays
       allocate( model%stress%efvs(1:0, 1:0, 1:0) )
       allocate( model%stress%tau%scalar(1:0, 1:0, 1:0) )
       allocate( model%stress%tau%xz(1:0, 1:0, 1:0) )
       allocate( model%stress%tau%yz(1:0, 1:0, 1:0) )
       allocate( model%stress%tau%xx(1:0, 1:0, 1:0) )
       allocate( model%stress%tau%yy(1:0, 1:0, 1:0) )
       allocate( model%stress%tau%xy(1:0, 1:0, 1:0) )
    endif
...
!!       call coordsystem_allocate(model%general%ice_grid, model%thckwk%float)  ! no longer used
    else
       allocate( model%geometry%thck_index(1:0, 1:0) )
       allocate( model%geomderv%dthckdtm(1:0, 1:0) )
       allocate( model%geomderv%dusrfdtm(1:0, 1:0) )
       allocate( model%thckwk%olds(1:0, 1:0, 1:0) )
       allocate( model%thckwk%oldthck(1:0, 1:0) )
       allocate( model%thckwk%oldthck2(1:0, 1:0) )
    endif
    ...
    if (model%options%whichdycore /= DYCORE_GLIDE) then   ! glam/glissade dycore
       ...
       allocate( model%geometry%age(1:0, 1:0, 1:0) )
       allocate( model%geomderv%dlsrfdew(1:0, 1:0) )
       allocate( model%geomderv%dlsrfdns(1:0, 1:0) )
       allocate( model%geomderv%staglsrf(1:0, 1:0) )
       allocate( model%geomderv%stagusrf(1:0, 1:0) )
       allocate( model%geomderv%stagtopg(1:0, 1:0) )
       allocate( model%geomderv%d2usrfdew2(1:0, 1:0) )
       allocate( model%geomderv%d2usrfdns2(1:0, 1:0) )
       allocate( model%geomderv%d2thckdew2(1:0, 1:0) )
       allocate( model%geomderv%d2thckdns2(1:0, 1:0) )
    endif
    ...
    if (model%options%gthf == GTHF_COMPUTE) then
       ...
       allocate( model%lithot%temp(1:0, 1:0, 1:0) )
       allocate( model%lithot%mask(1:0, 1:0) )
    endif
    ...
    if (model%options%isostasy == ISOSTASY_COMPUTE) then
       ...
       allocate( model%isostasy%load(1:0, 1:0) )
       allocate( model%isostasy%load_factors(1:0, 1:0) )
    endif
...
!!       allocate(model%basalproc%etill (ewn-1,nsn-1,model%basalproc%tnodes));model%basalproc%etill=0.5d0
       ...
       allocate( model%basalproc%mintauf(1:0, 1:0) )
    endif
